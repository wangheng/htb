...
<div class="filter">
	<form action="" method="get" class="form-inline">
		...
		<a href="<@spring.url '/backend/transactions' />" class="btn">清除</a>
...
<table class="table table-bordered table-hover table-condensed">
	...
	<#list transactions as transaction>
		<tr>
			...
			<td>
				<a href="<@spring.url '/backend/transactions/${transaction.id}' />">${transaction.name}</a>
			</td>
			<td>
			...
			<td>
				<#if transaction.updatable>
					| <a href="<@spring.url '/backend/transactions/${transaction.id}/edit' />">编辑</a>
				</#if>
				<#if transaction.deletable>
					<form action="<@spring.url '/backend/transactions/${transaction.id}' />" method="post" class="inline">
	...
	<ul>
		...
		<#list 1..paginator.last as p>
			<li class="<#if p == paginator.current>active</#if>">
				<a href="<@spring.url '/backend/transactions?p=${p}' />">${p}</a>
			</li>
		</#list>
		<li>
